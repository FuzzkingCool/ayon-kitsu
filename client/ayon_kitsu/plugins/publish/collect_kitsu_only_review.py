...
class CollectKitsuOnlyReview(KitsuPublishInstancePlugin):
    ...
    def process(self, instance):
        ...
        self.log.info(
            f"Configuring Kitsu-only review: {instance.data.get('productName', 'Unnamed')}"
        )
        ...
        for family in families_to_remove:
            if family in families:
                ...
                self.log.debug(
                    f"Removed '{family}' family to skip AYON integration"
                )
        ...
        # Ensure the instance will be processed by CollectKitsuEntities
        # by making sure it has the required data
        if not instance.data.get("folderEntity"):
            self.log.warning(
                "No folder entity found - Kitsu entities may not be collected"
            )
        if not instance.data.get("taskEntity"):
            self.log.warning(
                "No task entity found - Kitsu task may not be collected"
            )
        ...
        if (
            not representations
            and hasattr(instance, "transient_data")
            and "representations" in instance.transient_data
        ):
        ...
        self.log.info(
            f"Kitsu-only review configured for {folder_path}/{task_name}"
        )
