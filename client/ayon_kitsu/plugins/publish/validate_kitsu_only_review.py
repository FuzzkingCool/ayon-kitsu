...
class ValidateKitsuOnlyReview(
    ...
    def process(self, instance):
        ...
        self.log.info(f"Validating Kitsu-only review: {instance.data.get('productName', 'Unnamed')}")
        ...
        if not review_file:
            ...
            if file_size == 0:
                ...
                self.log.debug(f"Review file validated: {review_file} ({file_size} bytes)")
                
        # Validate Kitsu context
        kitsu_task = instance.data.get("kitsuTask")
        if not kitsu_task:
            errors.append("No Kitsu task context found. Ensure you are working in a valid AYON project with Kitsu integration.")
        else:
        ...
        if representations:
            ...
            self.log.debug("No representations found - this is expected for Kitsu-only reviews")
        ...
        if "kitsu" not in families:
            errors.append("Instance must have 'kitsu' family for Kitsu integration")
        ...
        if instance.data.get("enableComment", True):
            ...
            if not comment.strip():
                ...
                self.log.warning("Comment is enabled but no comment text provided. A default comment will be used.")
    ...
    def _validate_representation(self, representation, index):
        ...
        for field in required_repr_fields:
            if not representation.get(field):
                errors.append(f"Representation {index} missing required field: {field}")
                
        # Check staging directory exists
        staging_dir = representation.get("stagingDir")
        if staging_dir and not os.path.exists(staging_dir):
            errors.append(f"Representation {index} staging directory does not exist: {staging_dir}")
        ...
        if files:
            if isinstance(files, str):
                ...
                if not os.path.exists(file_path):
                    errors.append(f"Representation {index} file does not exist: {file_path}")
