...
### Image Location
```
ghcr.io/fuzzkingcool/ayon-kitsu-processor:latest
ghcr.io/fuzzkingcool/ayon-kitsu-processor:<version> 
```

Replace `fuzzkingcool` with your studio's GitHub organization name.
...
```bash
# Pull latest
docker pull ghcr.io/fuzzkingcool/ayon-kitsu-processor:latest

# Pull specific version
docker pull ghcr.io/fuzzkingcool/ayon-kitsu-processor:v1.2.6
```
...
### Optional
- `KITSU_URL` - Kitsu server URL (e.g. `https://studio-mdhr.cg-wire.com`). If set, overrides the `server` setting from AYON addon settings. Useful when addon settings aren't propagating correctly or for testing.
